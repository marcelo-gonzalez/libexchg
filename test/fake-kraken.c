                                ...
                                enum exchg_side side)
{
        ...
        for (int i = 0; i < n; i++) {
                ...
                buf_xsprintf(buf, "[ \"%s\", \"%s\", \"123.456\" ], ", price,
                             size);
        }
        buf_xsprintf(buf, "], ");
}
                           ...
                           struct exchg_test_event *msg)
{
        ...
        if (!k->channels[up->pair].subbed ||
            !k->channels[up->pair].status_sent) {
                                      ...
                                      const char *path, void *private)
{
        ...
        s->priv = kkn;
        struct kraken_proto *k = test_event_private(exchg_fake_queue_ws_event(
            s, EXCHG_EVENT_WS_PROTOCOL, sizeof(struct kraken_proto)));
                            ...
                            struct exchg_test_event *msg)
{
        ...
        if (msg->type == EXCHG_EVENT_ORDER_CANCEL_ACK) {
                ...
                if (cancel->err[0]) {
                        buf_xsprintf(buf,
                                     ...
                                     cancel->err);
                        if (cancel->reqid)
                                buf_xsprintf(buf, ", \"reqid\": %u",
                                             cancel->reqid);
                        buf_xsprintf(buf, "}");
                } else {
                        buf_xsprintf(buf, "{\"event\": \"cancelOrderStatus\", "
                                          "\"status\": \"ok\"");
                        if (cancel->reqid)
                                buf_xsprintf(buf, ", \"reqid\": %u",
                                             cancel->reqid);
        ...
        case ACK_OPENORDERS:
                ...
                case EXCHG_ORDER_CANCELED:
                        ...
                        break;
                default:
                        exchg_log("kraken-test: Don't know how to generate an "
                                  "event for order status %d\n",
                                  ack->status);
                        return;
                }
                         ...
                         struct order_cancel *cancel)
{
        ...
        struct websocket_conn *ws =
            fake_websocket_get(ctx, "ws-auth.kraken.com", NULL);
        ...
        LIST_FOREACH(o, &ctx->servers[EXCHG_KRAKEN].order_list, list)
        {
                ...
                if (uref != userref)
                        continue;
                if (decimal_cmp(&o->info.filled_size, &o->info.order.size) >=
                    0) {
...
static void private_ws_write(struct websocket_conn *w, const char *buf,
                             size_t len)
{
        ...
        for (int i = 0; i < pw->toks[0].size; i++) {
                ...
                if (json_streq(buf, key, "event")) {
                        ...
                        if (value->size != 1) {
                                exchg_log("FIXME: kraken test can only cancel "
                                          "one order at a time for now. Got "
                                          "this request:\n");
                                          ...
                                          void *private)
{
        ...
        struct http_conn *req = fake_http_conn_alloc(
            ctx, EXCHG_KRAKEN, EXCHG_EVENT_PAIRS_DATA, private);
                          ...
                          struct buf *buf)
{
        ...
        for (enum exchg_currency c = 0; c < EXCHG_NUM_CCYS; c++) {
                char s[30];
                decimal_t *balance =
                    &req->ctx->servers[EXCHG_KRAKEN].balances[c];
                                       ...
                                       void *private)
{
        ...
        struct http_conn *req = fake_http_conn_alloc(
            ctx, EXCHG_KRAKEN, EXCHG_EVENT_BALANCES, private);
        ...
        unsigned char *k =
            xzalloc((strlen(exchg_test_kraken_private) / 4) * 3 + 3);
        int state = 0;
        unsigned int save = 0;
        int len = g_base64_decode_step(exchg_test_kraken_private,
                                       strlen(exchg_test_kraken_private), k,
                                       &state, &save);
        if (len == 0) {
                fprintf(stderr,
                        "Kraken test could not base64 decode private apikey\n");
                exit(1);
        }
...
{
        struct http_conn *req = fake_http_conn_alloc(
            ctx, EXCHG_KRAKEN, EXCHG_EVENT_HTTP_PROTOCOL, private);
        req->read = token_read;
        req->write = no_http_write;
        req->add_header = no_http_add_header;
        req->destroy = fake_http_conn_free;
...
static void add_order_write(struct http_conn *req, const char *body, size_t len)
{
        ...
        while (1) {
                ...
                if (!strncmp(&body[key], "userref", strlen("userref"))) {
                        ...
                        ack->order.pair =
                            asset_name_to_pair(body, val, val_end);
                        if (ack->order.pair == -1) {
                                problem = "bad pair";
                                goto bad;
                        }
                        ...
                        if (decimal_from_str_n(&ack->order.price, &body[val],
                                               val_end - val)) {
                                problem = "bad price";
                                goto bad;
                        }
                        ...
                        if (decimal_from_str_n(&ack->order.size, &body[val],
                                               val_end - val)) {
                                problem = "bad size";
                                goto bad;
                        }
                ...
                } else if (!strncmp(&body[key], "timeinforce",
                                    strlen("timeinforce"))) {
        ...
        if (!request.userref) {
                exchg_log("FIXME: kraken test expects to get a \"userref\" "
                          "with orders for now. HTTP POST data:\n");
                fwrite(body, 1, len, stderr);
                fputc('\n', stderr);
                return;
        }
        ...
        struct test_order *o =
            on_order_placed(req->ctx, EXCHG_KRAKEN, ack, sizeof(int64_t));
        *(int64_t *)test_order_private(o) = request.userref;
        struct websocket_conn *ws =
            fake_websocket_get(req->ctx, "ws-auth.kraken.com", NULL);
        if (!ws)
                return;
...
{
        struct http_conn *req = fake_http_conn_alloc(
            ctx, EXCHG_KRAKEN, EXCHG_EVENT_ORDER_ACK, private);
