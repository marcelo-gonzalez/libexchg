...
static int check_intent(const char *exchange, enum exchg_pair pair,
			...
			const char *amount, bool sandbox) {
	...
	if (side == EXCHG_SIDE_BUY)
		sprintf(to_do, "buy %s %s paying in %s on %s", amount, exchg_ccy_to_str(base),
			exchg_ccy_to_str(price), exchange);
	else
		sprintf(to_do, "sell %s %s to receive %s on %s", amount, exchg_ccy_to_str(base),
			exchg_ccy_to_str(price), exchange);
	puts(to_do);
...
int main(int argc, char **argv) {
	...
	struct exchg_context *ctx = exchg_new(&trade_callbacks,
					      &exchg_opts, &state);
	if (!ctx)
		return 1;
