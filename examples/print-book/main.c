...
static void levels_init(struct book_data *b)
{
        ...
        memset(b->levels, 0, sizeof(struct screen_level) * b->cap);
}
                      ...
                      decimal_t *center)
{
        ...
        sub_increment(&t->low, center, t->price_decimals,
                      t->data.num_levels / 2 - 1);
        ...
        memset(t->data.levels, 0,
               sizeof(struct screen_level) * t->data.num_levels);
...
static void book_init(struct terminal_book *book, struct exchg_context *ctx)
{
        ...
        for (enum exchg_id i = 0; i < EXCHG_ALL_EXCHANGES; i++) {
                ...
                if (!exchg_pair_info_current(cl))
                        return;
                const struct exchg_pair_info *info =
                    exchg_pair_info(cl, book->pair);
                if (!info->available)
                        continue;
