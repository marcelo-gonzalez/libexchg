...
static int token_recv(struct exchg_client *cl, struct http *http, int status,
                ...
                if (json_streq(json, key, "error")) {
                        ...
                        for (int j = 0; j < n; j++) {
                                ...
                                if (json_streq(json, key, "token")) {
                                        ...
                                        key_idx = json_skip(num_toks, toks,
                                                            key_idx + 1);
                        }
                } else
                        key_idx = json_skip(num_toks, toks, key_idx + 1);
...
{
        if (websocket_printf(kkn->private_ws,
                             "{ \"event\": "
...
static int kraken_subscribe(struct kraken_client *kkn, enum exchg_pair pair)
{
        ...
        pi->subbed = true;
        if (websocket_printf(kkn->data_ws,
                             "{ \"event\": \"subscribe\", "
                             "\"pair\": [\"%s\"], "
...
static int parse_event(struct exchg_client *cl, const char *json, int num_toks,
                       jsmntok_t *toks)
{
        struct kraken_client *kkn = client_private(cl);
        const char *problem;
        ...
        for (int i = 0; i < toks[0].size; i++) {
                ...
                if (json_streq(json, key, "event")) {
                        ...
                        for (enum exchg_pair pair = 0; pair < EXCHG_NUM_PAIRS;
                             pair++) {
                                struct kraken_pair_info *p =
                                    &kkn->pair_info[pair];
                                if (!p->wsname)
                                        continue;
        ...
        if (status.type == EVENT_SUB_STATUS) {
                if (!json_streq(json, status.status, "subscribed")) {
                        // TODO: also probly should check that
                        // subscription:name: is "openOrders"
                        if (kkn->private_ws && !kkn->openorders_recvd &&
                            status.error_msg &&
                            json_streq(json, status.error_msg,
                                       "ESession:Invalid session")) {
                                exchg_log("Getting new token after "
                                          "subscription error:\n");
                                json_fprintln(stderr, json, &toks[0]);
                                free(kkn->ws_token);
                                kkn->ws_token = NULL;
                ...
                if (status.reqid == 0)
                        return 0;
                unsigned int id = GPOINTER_TO_UINT(g_hash_table_lookup(
                    kkn->cancelations, GUINT_TO_POINTER(status.reqid)));
                if (!id)
                        return 0;
                struct order_info *oi = exchg_order_lookup(cl, id);
                if (!oi) {
                        exchg_log("Kraken: Received order cancel update for "
                                  "unknown order:\n");
                        json_fprintln(stderr, json, &toks[0]);
                        cancel_free(cl, id);
                        return 0;
                              ...
                              enum exchg_pair pair, bool bids, bool is_initial)
{
        ...
        struct exchg_limit_order *order =
            bids ? &upd->bids[upd->num_bids] : &upd->asks[upd->num_asks];
        ...
        for (int i = 0; i < n; i++) {
                ...
                if (json_streq(json, key, "a")) {
                        idx = insert_orders_side(cl, json, num_toks, toks,
                                                 idx + 1, pair, false, false);
                } else if (json_streq(json, key, "as")) {
                        idx = insert_orders_side(cl, json, num_toks, toks,
                                                 idx + 1, pair, false, true);
...
static int kraken_recv(struct exchg_client *cl, struct websocket *w, char *json,
                       int num_toks, jsmntok_t *toks)
{
        ...
        if (json_get_int(&channel_id, json, &toks[1])) {
                exchg_log(
                    "Kraken sent update with bad first-element channel id:\n");
                json_fprintln(stderr, json, &toks[0]);
                return -1;
        }
        ...
        if (!g_hash_table_lookup_extended(kkn->channel_mapping,
                                          GINT_TO_POINTER(channel_id), NULL,
                                          &val)) {
...
static int kraken_conn_established(struct exchg_client *cl, struct websocket *w)
{
        ...
        for (int i = 0; i < toks[0].size; i++) {
                ...
                if (json_streq(json, key, "error")) {
                        ...
                        for (int j = 0; j < n; j++) {
                                ...
                                if (json_get_decimal(&balances[c], json,
                                                     value)) {
...
static int kraken_new_keypair(struct exchg_client *cl, const unsigned char *key,
                              size_t len)
{
        struct kraken_client *kc = client_private(cl);
...
static int parse_openorders(struct exchg_client *cl, const char *json,
                            int num_toks, jsmntok_t *toks)
{
        struct kraken_client *kkn = client_private(cl);
        const char *problem;
                           ...
                           char *json, int num_toks, jsmntok_t *toks)
{
        ...
        if (json_streq(json, type, "openOrders")) {
                ...
                exchg_log(
                    "Kraken privte websocket sent data on unknown channel:\n");
                json_fprintln(stderr, json, &toks[0]);
        }
        return 0;
                                ...
                                struct exchg_order_info *info)
{
        ...
        if (unlikely(!pi->available)) {
                exchg_log("Kraken can't submit order in %s. Pair not available "
                          "on Kraken\n",
        ...
        if (websocket_printf(kkn->private_ws,
                             ...
                             info->order.side == EXCHG_SIDE_BUY ? "buy"
                                                                : "sell",
                             kpi->wsname, px, sz, timeinforce) < 0)
                return -1;
...
static int http_post_add_order(struct exchg_client *cl, struct order_info *oi,
                               bool update_on_err)
{
        ...
        if (kraken_body_sprintf(
                http,
                "userref=%" PRId64 "&ordertype=limit&"
                "type=%s&pair=%s&price=%s&volume=%s",
                info->id, info->order.side == EXCHG_SIDE_BUY ? "buy" : "sell",
                kraken_pair_to_str(info->order.pair), px, sz) < 0) {
                if (update_on_err)
                                  ...
                                  void *request_private)
{
        ...
        if (unlikely(!cl->pair_info_current)) {
                if (get_pair_info(cl) ||
                    queue_work(cl, place_order_work, info)) {
...
static int kraken_cancel_order(struct exchg_client *cl, struct order_info *info)
{
        ...
        if (websocket_established(kkn->private_ws) && kkn->ws_token) {
                ...
                g_hash_table_insert(
                    kkn->cancelations, GUINT_TO_POINTER(reqid),
...
struct exchg_client *alloc_kraken_client(struct exchg_context *ctx)
{
        ...
        struct exchg_client *ret = alloc_exchg_client(
            ctx, EXCHG_KRAKEN, "SHA512", 2000, sizeof(struct kraken_client));
        if (!ret)
                return NULL;
