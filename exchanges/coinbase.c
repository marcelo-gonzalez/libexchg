...
static int msg_finish(struct exchg_client *cl, struct ws_msg *msg,
	...
	case TYPE_RECEIVED:
		if (!msg->client_oid) {
			*problem = "\"received\" message with no \"client_oid\"";
			return -1;
		}
		if (!msg->order_id) {
		...
		if (client_oid < 0) {
			*problem = "\"client_oid\" in \"received\" message does not"
				" appear to have been generated by us";
			return -1;
		}
		info = exchg_order_lookup(cl, client_oid);
		if (!info) {
			exchg_log("Coinbase: Received order update with unknown "
				  "client_oid:\n");
	...
	case TYPE_MATCH:
		...
		if (!info) {
			exchg_log("Coinbase: received \"match\" message with unknown orders:\n");
			json[msg->taker_oid->end] = '\"';
			json[msg->maker_oid->end] = '\"';
			json_fprintln(stderr, json, first_tok);
			return 0;
		}
	...
	case TYPE_DONE:
		...
		if (!info) {
			...
			exchg_log("Coinbase: received order update with unknown order id:\n");
			json_fprintln(stderr, json, first_tok);
			return 0;
		}
