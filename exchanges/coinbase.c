...
static int parse_changes(struct exchg_client *cl, struct ws_msg *msg,
                         const char *json, jsmntok_t *toks, int idx,
                         const char **problem)
{
        ...
        for (int i = 0; i < toks[idx].size; i++) {
                ...
                order = is_bid ? &upd->bids[upd->num_bids]
                               : &upd->asks[upd->num_asks];
                (*num_orders)++;

                order->exchange_id = EXCHG_COINBASE;
                if (unlikely(json_get_decimal(&order->price, json,
                                              &toks[data_idx + 2]))) {
                        *problem = "bad price";
                        return -1;
                }
                if (unlikely(json_get_decimal(&order->size, json,
                                              &toks[data_idx + 3]))) {
                        *problem = "bad size";
                        return -1;
                }
...
static int parse_snapshot(struct exchg_client *cl, struct ws_msg *msg,
                          bool is_bids, const char *json, jsmntok_t *toks,
...
static int msg_finish(struct exchg_client *cl, struct ws_msg *msg, char *json,
                      jsmntok_t *first_tok, const char **problem)
{
        struct coinbase_client *cb = client_private(cl);
        ...
        case TYPE_MATCH:
                if (!msg->maker_oid) {
                        *problem =
                            "\"received\" message with no \"maker_order_id\"";
                        return -1;
                }
                if (!msg->taker_oid) {
                        *problem =
                            "\"received\" message with no \"taker_order_id\"";
                        return -1;
                }
                if (!msg->got_size) {
                ...
                info = g_hash_table_lookup(cb->orders,
                                           &json[msg->taker_oid->start]);
                if (!info) {
                        json[msg->maker_oid->end] = 0;
                        info = g_hash_table_lookup(
                            cb->orders, &json[msg->maker_oid->start]);
                }
                if (!info)
                        return 0;
        ...
        case TYPE_DONE:
                if (!msg->order_id) {
                        *problem =
                            "\"open\" or \"done\" message with no \"order_id\"";
                        return -1;
                }
                if (!msg->got_remaining_size) {
                        *problem = "\"open\" or \"done\" message with no "
                                   "\"remaining_size\"";
                        return -1;
                }
                json[msg->order_id->end] = 0;
                info = g_hash_table_lookup(cb->orders,
                                           &json[msg->order_id->start]);
                if (!info)
                        return 0;
                ...
                if (decimal_is_negative(&filled_size)) {
                        ...
                        exchg_log(
                            "Coinbase indicates %s remaining for order %" PRId64
                            ". This is greater than the original full size: "
                            "%s\n",
                ...
                if (msg->type == TYPE_OPEN) {
                        ...
                        if (!msg->reason) {
                                exchg_log("Coinbase sent \"done\" message with "
                                          "no \"reason\":");
...
static int ws_recv(struct exchg_client *cl, struct websocket *w, char *json,
        ...
        for (int j = 0; j < toks[0].size; j++) {
                ...
                if (json_streq(json, key, "type")) {
                        if (json_streq(json, value, "snapshot")) {
                                ...
                                cb->sub_acked = true;
                                // TODO: actually parse it to see if the user
                                // channel is there
                                ...
                                exchg_log("Coinbase websocket sent "
                        ...
                        if (unlikely(
                                sscanf(&json[value->start],
                                       "%d-%d-%dT%d:%d:%d.%dZ", &tm.tm_year,
                        ...
                        if (json_get_decimal(&msg.remaining_size, json,
                                             value)) {
...
static int coinbase_auth(struct coinbase_auth *auth, struct hmac_ctx *hmac_ctx,
                         const char *path, const char *method, const char *body,
                         size_t body_len)
{
        ...
        auth_end += body_len;
        return hmac_ctx_b64(hmac_ctx, to_auth, auth_end - to_auth, auth->hmac,
                            &auth->hmac_len);
}
...
static int channel_sub(struct exchg_client *cl)
{
        ...
        if (user_sub) {
                ...
                if (coinbase_auth(&auth, &cl->hmac_ctx, "/users/self/verify",
                                  "GET", NULL, 0))
                        return -1;
        ...
        if (websocket_printf(cb->ws,
                             "{ \"type\": \"subscribe\", "
                             "\"channels\": [%s%s%s]%s}",
                             level2, level2_sub && user_sub ? "," : "", user,
                             auth_fields) < 0)
                return -1;
        return 0;
}
...
static int ws_on_disconnect(struct exchg_client *cl, struct websocket *ws,
                      ...
                      char *json, int num_toks, jsmntok_t *toks)
{
        ...
        for (int i = 0; i < toks[0].size; i++) {
                ...
                if (toks[obj_idx].type != JSMN_OBJECT) {
                        if (!non_obj_warned) {
                                exchg_log("%s%s gave non-object array member "
                                          "(idx %d):\n",
                ...
                pi->available = true;
                // TODO: the min size logic for coinbase is now based on price *
                // size should update the pair info struct to be able to
                // describe that. for now just set it to zero.
...
static int private_add_headers(struct exchg_client *cl, struct http *http)
{
        ...
        if (http_add_header(http, (unsigned char *)"CB-ACCESS-TIMESTAMP:",
                            (unsigned char *)data->auth.timestamp,
                            data->auth.timestamp_len))
                return 1;
...
static int coinbase_http_auth(struct coinbase_auth *auth,
                              struct hmac_ctx *hmac_ctx, struct http *http)
{
                       ...
                       char *json, int num_toks, jsmntok_t *toks)
{
        ...
        for (int i = 0; i < toks[0].size; i++) {
                ...
                if (json_streq(json, key, "status")) {
                        if (json_streq(json, value, "pending")) {
                                ...
                                exchg_log("Coinbase received order ack with "
                                          "unrecognized \"type\":\n");
                                json_fprintln(stderr, json, &toks[0]);
                                ack.status = EXCHG_ORDER_PENDING;
                        }
        ...
        if (status != 200)
                ...
                if (!ack.order_id) {
                        exchg_log(
                            "Coinbase sent order upate with no \"id\":\n");
                        json_fprintln(stderr, json, &toks[0]);
                } else if (set_order_id(client_private(cl), info, json,
                                        ack.order_id))
                       ...
                       struct order_info *oi, bool update_on_err)
{
        ...
        if (http_body_sprintf(
                ...
                "\"time_in_force\": \"%s\"}",
                oid, cb_info->id,
                info->order.side == EXCHG_SIDE_BUY ? "buy" : "sell", price,
                size, info->opts.immediate_or_cancel ? "IOC" : "GTC") < 0) {
                if (update_on_err) {
                        snprintf(info->err, EXCHG_ORDER_ERR_SIZE,
                                 "Out-Of-Memory");
                        order_update(cl, oi, EXCHG_ORDER_ERROR, NULL, false);
                             ...
                             jsmntok_t *toks)
{
        if (num_toks > 1) {
                ...
                if (oi) {
                        ...
                        // TODO: would make sense for http_retry() to take care
                        // of redoing the auth stuff
                        if (!c->cancel_retried &&
                            !coinbase_http_auth(&data->auth, &cl->hmac_ctx,
                                                http)) {
                                http_retry(http);
                ...
                exchg_log("Cancelation of order %" PRId64
                          " failed. %sReceived:\n",
                          data->id, retrying);
                json_fprintln(stderr, json, &toks[0]);
        }
                                 ...
                                 const struct exchg_request_options *options)
{
        ...
        // note that since the order status is not UNSUBMITTED,
        // cl->pair_info_current is true and cb->pair_info[pair].id is valid
        sprintf(&path[strlen("/orders/client:") + 36], "?product_id=%s",
