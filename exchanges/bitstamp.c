...
static int fill_orders(struct exchg_client *cl, const char *json,
		       int num_toks, jsmntok_t *toks, int idx,
		       struct bitstamp_msg *msg, bool is_bids) {
	const char *problem;
	...
	case EVENT_DATA:
		...
		if (!(pi->state & BTS_BOOK_COMPLETE)) {
			...
			if (msg->chan_type == CHAN_DIFF) {
				...
				if (pi->first_full_update == 0 || msg->timestamp <= pi->first_full_update)
					return 1;
				do_update(cl, msg);
			...
			// wait until we know there are no previous diff updates relevant
			// to this full snapshot. Could avoid returning here by writing code
			// to remember the old diffs until we establish a working book but
...
static int bitstamp_recv(struct exchg_client *cl, struct conn *conn,
	...
	for (int key_idx = 1; key_idx < num_toks;) {
		...
		if (json_streq(json, key, "data")) {
			...
			if (json_streq(json, value, "bts:subscription_succeeded")) {
				// Nothing to do...
				return 0;
			...
			exchg_log("Bitstamp requested reconnect. Reconnecting...\n");
			return -1;
...
static int parse_info_token(struct bitstamp_pair_info *info, char *json,
	...
	if (json_streq(json, key, "base_decimals")) {
		...
		if (decimal_from_str_n(&info->min_size, &json[value->start], len)) {
			sprintf(problem, "bad minimum_order field");
			return -1;
		}
		...
		if (!exchg_strn_to_ccy(&info->min_currency,
				       c, &json[value->end]-c))
...
static int bitstamp_parse_info(struct exchg_client *cl, struct conn *conn,
	...
	for (int i = 0; i < toks[0].size; i++) {
		...
		for (int j = 0; j < tok->size; j++) {
			key_idx = parse_info_token(&info, json, num_toks,
		...
		if (info.dont_care)
			continue;
		if (info.pair == -1) {
			sprintf(problem, "pair info without a url_symbol field");
			goto out_bad;
		}
		if (info.base_decimals == -1 || info.price_decimals == -1) {

			sprintf(problem, "pair info with incomplete decimals info");
			goto out_bad;
		}
		if (!info.min_order_good) {
			sprintf(problem, "pair info without valid minimum_order field");
			goto out_bad;
		}
		if (!info.enabled) {
			exchg_log("bitstamp indicates trading not enabled for %s.\n",
...
static int balances_recv(struct exchg_client *cl, struct conn *conn,
	...
	for (int i = 0; i < toks[0].size; i++) {
		...
		if (key->end - key->start != strlen("_available") + 3 ||
		    memcmp("_available", &json[key->start+3], strlen("_available")) ||
...
static int string_to_sign(char *dst, struct exchg_client *cl, const char *millis,
	...
	if (!payload)
		payload = "";
	return sprintf(dst, "%sPOST%s%s%s%s%sv2%s",
		       bts->api_header, host, path,
	...
	for (int i = 0; i < toks[0].size; i++) {
		...
		if (json_streq(json, key, "status")) {
			...
			if (value->type != JSMN_OBJECT) {
				json_strncpy(info->err, json, value, EXCHG_ORDER_ERR_SIZE);
				key_idx = json_skip(num_toks, toks, key_idx+1);
			} else {
				int next_key = json_skip(num_toks, toks, key_idx+1);
				key_idx += 2;
