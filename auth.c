...
int hmac_ctx_setkey(struct hmac_ctx *ctx, const unsigned char *key, size_t keylen) {
...
static size_t __base64_encode(char *dst, const unsigned char *in, int len) {
	int state = 0, save = 0;
	size_t outlen = g_base64_encode_step(in, len, FALSE, dst, &state, &save);
...
static size_t hmac_ctx_mac(struct hmac_ctx *ctx, const unsigned char *msg, size_t len,
			   unsigned char *hmac, size_t *hmac_len, size_t hmac_size) {
...
int hmac_ctx_hex(struct hmac_ctx *ctx,
...
int hmac_ctx_b64(struct hmac_ctx *ctx,
		 const unsigned char *msg, size_t len,
