...
void fill_order(struct exchg_limit_order *order, struct order_data *data,
                int exchange_id)
{
        ...
        if (data->is_bid) {
                decimal_dec_bps(&order->net_price, &order->price, 50, 8);
        } else {
                decimal_inc_bps(&order->net_price, &order->price, 50, 8);
        }
}
