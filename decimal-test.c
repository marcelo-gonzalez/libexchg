...
const static struct {
    ...
    {"50000000000.00000", "1.0026", "50000000001.00260", "49999999998.99740",
     "50130000000.0000"},
};
        ...
        for (int i = 0; i < ARRAY_SIZE(arithmetic_tests); i++) {
                ...
                if (strcmp(a, arithmetic_tests[i].want_add)) {
                        fprintf(stderr, "%s + %s = %s, want %s\n",
                                arithmetic_tests[i].a, arithmetic_tests[i].b, a,
                                arithmetic_tests[i].want_add);
                        ret = 1;
                }
                if (strcmp(s, arithmetic_tests[i].want_sub)) {
                        fprintf(stderr, "%s - %s = %s, want %s\n",
                                arithmetic_tests[i].a, arithmetic_tests[i].b, s,
                                arithmetic_tests[i].want_sub);
                        ret = 1;
                }
                if (strcmp(m, arithmetic_tests[i].want_mul)) {
                        fprintf(stderr, "%s * %s = %s, want %s\n",
                                arithmetic_tests[i].a, arithmetic_tests[i].b, m,
                                arithmetic_tests[i].want_mul);
                        ret = 1;
                }
        }
}
...
static struct {
    ...
    {{0, 123}, {0, 123}, 0},    {{1, 123}, {0, 123}, -1},
    {{0, 123}, {1, 123}, 1},    {{0, 1234}, {0, 123}, 1},
    {{0, 123}, {0, 1234}, -1},  {{1, 1230}, {0, 123}, 0},
    {{3, 123000}, {0, 123}, 0}, {{3, 123456}, {2, 12346}, -1},
    {{0, 123}, {1, 12}, 1},     {{0, -123}, {0, 123}, -1},
    {{5, -123}, {0, 123}, -1},  {{0, -123}, {5, 123}, -1},
};
...
void test_decimal_cmp(void)
{
        for (int i = 0; i < ARRAY_SIZE(cmp_tests); i++) {
                ...
                if (cmp != cmp_tests[i].want_cmp) {
                        fprintf(stderr,
                                "decimal_cmp({%d, %" PRId64 "}, {%d, %" PRId64
                                "}) "
                                "wanted %d, got %d\n",
                                a->places, a->value, b->places, b->value,
                                cmp_tests[i].want_cmp, cmp);
                        ret = 1;
                }
        }
}
        ...
        for (int i = 0; i < ARRAY_SIZE(to_fractional_tests); i++) {
                ...
                if (got_0 != test->want_0_places ||
                    ...
                    got_6 != test->want_6_places) {
                        fprintf(stderr,
                                "error: decimal_to_fractional(%s): 0 -> %ld, "
                                "3 -> %ld, 6 -> %ld\n",
                                test->value, got_0, got_3, got_6);
                        ret = 1;
                }
        }
}
...
void test_bps(void)
{
        for (int i = 0; i < ARRAY_SIZE(bps_tests); i++) {
                ...
                if (decimal_cmp(&up, &want_up)) {
                        ...
                        decimal_to_str(s, &up);
                        fprintf(stderr,
                                "decimal_inc_bps(%s, %d, %d) = %s, want %s\n",
                ...
                if (decimal_cmp(&down, &want_down)) {
                        ...
                        fprintf(stderr,
                                "decimal_dec_bps(%s, %d, %d) = %s, want %s\n",
...
void test_decimal_from_str_n(void)
{
        for (int i = 0; i < ARRAY_SIZE(from_str_tests); i++) {
                ...
                if (!!err != !!from_str_tests[i].want_err) {
                        fprintf(
                            stderr, "%s: test %d, got err %d, want err %d\n",
                ...
                if (cmp != 0) {
                        fprintf(
                            stderr,
                            "decimal_from_str_n(\"%s\", %zu) comparison with"
                            " expected not zero. Got { %d, %" PRId64
                            " }. Comparison %d\n",
...
static struct trunc_test {
    ...
    {"12", 0, "12"},           {"12", 2, "12"},
    {"1234.12", 8, "1234.12"}, {"1234.12", 1, "1234.1"},
    {"1234.12", 0, "1234"},    {"922337203685.4775807", 3, "922337203685.477"},
};
...
static struct trunc_test trim_tests[] = {
    {"12.0", 1, "12.0"},     {"12.0", 0, "12"},
    {"12.5600", 2, "12.56"}, {"12.5610", 2, "12.561"},
    {"12.5000", 2, "12.50"}, {"12.50001", 2, "12.50001"},
    {".5000", 2, "0.50"},
};
