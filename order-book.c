...
static void insert_order(struct order_book *ob, const struct exchg_limit_order *order, bool is_bid) {
	GTree *tree;
...
bool order_book_best_bid(struct exchg_limit_order *dst,
			 struct order_book *ob, enum exchg_id id) {
...
bool order_book_best_ask(struct exchg_limit_order *dst,
			 struct order_book *ob, enum exchg_id id) {
	if (id == EXCHG_ALL_EXCHANGES)
...
struct order_book *order_book_new(int max_depth[EXCHG_ALL_EXCHANGES], bool sort_by_nominal_price) {
	...
	for (int i = 0; i < EXCHG_ALL_EXCHANGES; i++) {
		...
		ob->per_exchg[i].bids = g_tree_new_full(ob->bids_cmp, NULL, NULL, NULL);
		ob->per_exchg[i].asks = g_tree_new_full(ob->asks_cmp, NULL, NULL, NULL);
	}
	return ob;
}
